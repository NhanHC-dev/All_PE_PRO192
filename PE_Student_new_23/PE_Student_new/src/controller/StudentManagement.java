...
public class StudentManagement extends Menu<String> {
    ...
    private void searchStudent() {
        String[] searchOptions = {"Search by ID", "Search by Name", "Search by GPA", "Return"};
        new Menu<String>("Search Student", searchOptions) {
            ...
            public void execute(int choice) {
                switch (choice) {
                    ...
                    case 3 -> {
                        Float gpa = val.getFloat("Enter gpa: ");
                        studentList.listAllStudents(studentList.search(s -> s.getGPA() == gpa));
                    }
                    default -> {
                        this.stop();
    ...
    private void listAllAgeOfStudent(StudentList ArrayList) {
        ...
        for (int age = 0; age < studentsByAge.size(); age++) {
            ...
            if (!studentsWithAge.isEmpty()) {
                ...
                studentList.listAllStudents(studentsWithAge);
                System.out.println();
            }
        }
        System.out.println("------------------------------");
